...
引数:
- Memoファイル名（省略可）: 変換元のファイル。省略時は `00_Memo/` 内の全ての `.md` ファイルを対象に一括変換

実行内容:
1. 対象となるメモファイルを決定（引数があればそのファイル、なければ `00_Memo/**/*.md` の全ファイル）
2. 対象メモの内容を全て読み込み、分析して整理
...
### ステップ1: ファイルの検索とリスト化
1. `00_Memo/` 内の全ての `.md` ファイルを `glob_file_search` で検索（日付フォルダ内も含む、パターン例: `00_Memo/**/*.md`）
2. ファイル一覧を取得
...
```
/memo-to-inbox                    # 00_Memo内の全 .md ファイルを一括変換（自動実行）
